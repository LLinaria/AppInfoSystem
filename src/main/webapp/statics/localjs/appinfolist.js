...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href="/dev/app/appversionadd/"+obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href="/dev/app/appversionmodify/"+ versionid + "/"+ appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="/dev/app/appinfomodify/"+ obj.attr("appinfoid");
	}else{
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	window.location.href="/dev/app/appview/"+ obj.attr("appinfoid");
});
